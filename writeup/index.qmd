...
# (data downloaded from osf (saved by datapipe))
...
I found/did not find that participants rated the probability of joint success in round 3 as non-zero (_t_(`r ttest_result$parameter`) = `r ttest_result$statistic`, _p_ = `r signif(ttest_result$p.value, 2)`). See the black point range (data) for scenario "F,F;F,F".
...
fig3c_behavioral_only <- dat %>%
  ggplot(aes_string("scenario", "prob", group = 'model', color = 'model')) +
