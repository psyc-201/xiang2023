...
# render_raw <- function(htest) {
#   result <- unclass(htest) |> print() |> capture.output()
#   result_html_inner <- paste(result, collapse = "\n") |>
...
I found/did not find that the joint effort model was the only model which qualitatively predicted both that joint success is non-zero (the result in confirmatory analysis) and that probability of joint success increases as individual success increases.
...
ggsave('./writeup/replication-fig3c_behavioral_only.png', plot = fig3c_behavioral_only)
```

Original (Figure 3C) | New
---- | ----
![](./figure3c-legend.png)<br>![Figure 3C](./figure3c.png)<br>"Note. ... Model simulations averaged over 10 runs. Error bars indicate bootstrapped 95% confidence intervals." (p. 1571) | ![](./replication-fig3c_behavioral_only.png)
