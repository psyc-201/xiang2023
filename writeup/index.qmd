...
9 participants is completely feasible. To compensate for the original study's measured effect size being inflated (e.g., due to publication bias), 15 participants should be necessary for sufficient power for the primary effect of interest.

However, for the sake of qualitatively replicating the overall pattern of estimated probabilities as scenarios feature more successful lifts (monotonic increase in probability from left to right in Figure 3C; which I've labeled "B" above), and that the joint effort model is the only considered model which captures both the key effect and this pattern, I find it prudent to retain the same sample size so that I may attempt to replicate the original qualitative finding with equivalent precision. (Targeting higher precision could potentially counter aforementioned effect inflation present in the original paper, but as it is unclear how to quantify the necessary precision for these qualitative analyses, targeting the original precision appears to be a sensible option.)
