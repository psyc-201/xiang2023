...
The original effect size is _d_ = `r papaja::apa_num(original.cohensd$Cohens_d)`. <!-- d = 1.47 --> To achieve 80%, 90%, or 95% power to detect that effect size, I needed:
...
Further, in the original study, given 50 participants, the key effect appeared quite robust, with the CI for the data from scenario "F,F;F,F" being quite far from the line LiftProbability=0 (see Figure 3C), suggesting that with 50 participants, I was nearly guaranteed to replicate this effect if it is true.
...
4. See @sec-method-changes for an additional change
...
#### Differences from pre-data collection methods plan {#sec-method-changes}
...
I found that the joint effort model was the only model which not only predicted that joint success has non-zero probability (the result in confirmatory analysis), but also which qualitatively predicted that the probability of joint success increases as individual success increases.
...
The primary, confirmatory result fully replicated. The exporatory (qualitative) result also replicated.
