...
# (also include participants who have a session ID, as those who don't were not real Prolific participants)
dat_raw_filtered <- dat_raw_annotated %>%
  filter(passed_an_attention_check) %>%
  group_by(subjectId) %>% filter(any(!is.na(sessionId))) %>% ungroup()
...
if (interactive()) { # safeguard to help ensure this is not output by quarto
  ...
  dat_raw_annotated %>%
    select(subjectId, encryptedProlificId, bonus, passed_an_attention_check) %>%
    group_by(subjectId) %>%
    filter(!is.na(bonus)) %>%
    ungroup() %>%
...
# ggsave('./writeup/replication-fig3c_behavioral_only.png', plot = fig3c_behavioral_only, width=4, height=3)
...
---- | ----
![Figure 3C](./figure3c.png)<br>![](./figure3c-legend.png){height=150px}<br><br>"Note. ... Model simulations averaged over 10 runs. Error bars indicate bootstrapped 95% confidence intervals." (p. 1571) | ![](./replication-fig3c_behavioral_only.png)
