...
dat_raw_filtered <- dat_raw_annotated %>%
  ...
  filter(datapipe_meta.attributes.date_modified > lubridate::ymd("2025-11-20", tz="PST"))
...
if (interactive()) { # safeguard to help ensure this is not output by quarto
  ...
  dat_raw_annotated %>%
    ...
    filter(datapipe_meta.attributes.date_modified > lubridate::ymd("2025-11-20", tz="PST")) %>% # filter to show only post-pilot participants
...
dat_clean %>%
  ...
  geom_col(aes(fill = subject), stat="count")
...
dat_raw |> filter(trial_type == "survey-multi-choice") |> group_by(subjectId) |> mutate(attempts = n()) |> rowwise() |> mutate(responses = list(jsonlite::fromJSON(responses))) |> select(subjectId, responses, attempts) |> unnest_longer(responses) |>
  ...
  # filter(attempts > 2) |>
  ggplot(aes(x=responses, group=paste(subjectId, times_answered), fill=times_answered, color=subjectId)) + geom_col(stat="count") + facet_wrap(~responses_id, scales = "free") + theme(axis.text.x = element_text(angle = 20))
```
...
cohensd <- effectsize::effectsize(ttest_result)
```

I found that participants rated the probability of joint success in round 3 as non-zero (_t_(`r ttest_result$parameter`) = `r ttest_result$statistic`, _p_ = `r signif(ttest_result$p.value, 2)`). See the black point range (data) for scenario "F,F;F,F" under Exploratory analyses.

The effect size found is _d_ = `r round(cohensd$Cohens_d,2)`.
...
fig3c_w_safejoint <- fig3c +
  ...
  scale_color_manual(name = NULL, labels = c('Data','Joint effort model','Safe joint effort model','Solitary effort model','Compensatory effort model'),
      values = c('#000000','#e35d5e','#a51212ff','#004385','#05b2dc','#04a777'),
...
Original (Figure 3C) | New (Figure 3C Replication)
---- | ----
![Figure 3C](./figure3c.png)<br>![](./figure3c-legend.png){height=150px}<br><br>"Note. ... Model simulations averaged over 10 runs. Error bars indicate bootstrapped 95% confidence intervals." (p. 1571) | ![Figure 3C Replication](./replication-fig3c.png)<br><br>Note. Model simulations consider strengths from 1 to 10 (inclusive) in steps of 0.03 (determined by memory constraints). Error bars indicate bootstrapped 95% confidence intervals.

::: {.callout-note collapse="true"}
## Figure 3C Replication with Raw Data
![Figure 3C Replication with Raw Data](./replication-fig3c-withrawdata.png)
:::

::: {.callout-note collapse="true"}
## Figure 3C Replication with Safe Joint Model
![Figure 3C Replication with Safe Joint Model](./replication-fig3c-withsafejoint.png)
:::
