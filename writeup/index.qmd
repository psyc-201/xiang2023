...
Based on my power and precision considerations, I will collect 50 participants on Prolific, following the specific exclusion strategy reported below.
...
osfr::osf_retrieve_node(OSF_NODE_REPLICATION_PROJECT) %>%
...
dat_raw <- list.files(path = './data/', pattern = 'index1_.*_output.csv', full.names = T) %>%
  map_dfr(~ read.csv(.x, header = T, stringsAsFactors = F) %>%
            mutate(subjectId = str_extract(basename(.x), '(?<=index1_).*(?=_output.csv)')))

#### Data exclusion / filtering
# based on attention checks criterion
# (also remove low-attention pilots used for simply testing the task)
test_pilot_subjectIds <- c("6bt3juzg6gt3")
dat_raw <- dat_raw %>%
  group_by(subjectId) %>%
  mutate(failed_both_attention_checks = attention_sum[!is.na(attention_sum)] == 0) %>%
  ungroup() %>%
  filter(!failed_both_attention_checks) %>%
  filter(!(subjectId %in% test_pilot_subjectIds))
...
critical_tvalue <- qt(0.975, df=ttest_result$parameter)
...
fig3c_behavioral_only <- dat %>%
  ...
  geom_line(aes(group=subject), linetype="dashed", alpha=.5, data = dat %>% filter(round == 3 & agent == 'A')) +
