...
- Ps thought that collaborators could achieve a task jointly even if they have previously failed the task individually (`r papaja::apa_print(original.ttest_result)$statistic`; $d = `r papaja::apa_num(original.cohensd$Cohens_d)`$)
...
Collaborators could achieve a task jointly even if they have previously failed the task individually (`r papaja::apa_print(original.ttest_result)$statistic`; $d = `r papaja::apa_num(original.cohensd$Cohens_d)`$) | **Replicated!** (`r papaja::apa_print(replication.ttest_result)$statistic`; $d = `r papaja::apa_num(replication.cohensd$Cohens_d)`$)

- Replicated: joint success is possible even if there is no prior evidence of success

## Replication Results

<!-- TODO: style this slide and the previous slide better to fit all content (make plots smaller?); include legend in plots themselves --> 
