...
- Fundamental to human success is how we've evolved to effectively collaborate with one another [@Tomasello2005]
- Requires judgments of what potential collaborators could bring to the table, and judging how they will calibrate their effort during collaboration
- How complicated is all this?

::: {.notes}
Collaboration is fundamental

Fundamental to human success is how we've evolved to effectively collaborate with one another [@Tomasello2005]. There are many cognitive processes involved for being able to collaborate effectively.

For example, we judge how good others might be as potential collaborators in order to make decisions about who to work with. We then calibrate our own effort accordingly.

But as usual, evolution prefers the simplest solution to problems: so how complicated is our solution?
:::

## We judge what others could bring to the table
...
::: {.notes}
We need to consider how we judge what others could bring to the table
...
## We judge what others could bring to the table, and how they _calibrate effort_

- Simple solutions for calibrating effort:
  - _Social compensation_: we simply think the other person won't apply any effort, so we allocate effort like we were working alone [@Williams1991-sv] ("solitary" model)
...
- Proposed model in @Xiang2023-zo:
  - _Jointly optimize effort_: we infer how much effort the other will allocate and how much they think _we_ will allocate, and we adjust our own effort accordingly, while also trying to be fair ("joint effort" model)

::: {.notes}
After we infer each other's competence and effort from observing their behavior, how do we set our own effort when collaborating with them?

There are a couple simple solutions that evolution could have chosen -> &lt;read slide&gt;

The authors propose a more complicated model -> &lt;read slide&gt;
...
- @Xiang2023-zo used a game-like task to collect participants' judgments of competence, effort, and likelihood of success of virtual collaborators, in order to see how well the "joint effort" model predicted these judgments
  <iframe src="https://psyc-201.github.io/xiang2023/exp/index1.html?TEST" style="zoom: .75; width: 133%; height: 20em"></iframe>

::: {.notes}
The authors then sought to validate their more complicated model
...
2. Participants also thought **collaborators' chance of succeeding together increased with more individual successes** (i.e., collaborators inferred each other's competence and effort from prior attempts), and that it was **gradual** (i.e., collaborators tried to distribute effort fairly)
3. **Only the proposed "joint effort" model predicted both:**

::: {.fragment style="font-size: 60%; text-align: center"}
![(Preview) Notice `F,F;F,F`, the left-to-right trend, and that the red line fits the best.](../writeup/original-fig3c.png){height=250 style="--r-block-margin: 0"}
:::

::: {.notes}
&lt;Read slide&gt;

evidence suggests that their model is worth the complexity!
:::
...
- For the computational model, I chose to reimplement the model in `memo`, a new probabilistic programming language [@chandra2025memo] for performance and extensibility advantages (for future work) 
  ...
  - Accomplished this by changing one implementation variable at a time and measuring its impact to characterize where discrepancies came from
...
Collaborators' chance of succeeding together gradually increased with more indiv success, validating joint model
:::
...
- Replicated: joint success is possible even if there is no prior evidence of individual success, AND joint success likelihood gradually increases as cases of individual success increases
...
- I observed slight differences:
  - larger CIs
  - a flat section of the curve (i.e. similarity between two scenarios) not seen before
...
Maybe the greater variance in the data (driving the larger CIs) is due to this heterogeneity that might not have been present in the original data. or just due to publication bias/effect inflation
:::
